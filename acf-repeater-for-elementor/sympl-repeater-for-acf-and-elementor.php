<?php
/*
Plugin Name: Sympl Repeater for ACF and Elementor
...
Text Domain: acf-repeater-for-elementor
...
function symp_conditional_plugin_activation_check() {
...
register_activation_hook( __FILE__, 'symp_conditional_plugin_activation_check' );

// Prevent plugin from running if dependencies are not met
function symp_conditional_plugin_dependency_check() {
...
add_action( 'admin_init', 'symp_conditional_plugin_dependency_check' );


// Add the ACF Repeater settings to Elementor widgets - use Elementor_ACF_Repeater_Settings class
$elementor_acf_repeater_settings = new Sympl_Elementor_ACF_Field_Settings();


add_action( 'elementor/widgets/register', function( $widgets_manager ) {
    require_once __DIR__ . '/widgets/arfe-repeater-carousel.php';
    $widgets_manager->register( new \SYMP_Repeater_For_Elementor\Widgets\SYMP_Repeater_Carousel() );
} );

add_action('elementor/frontend/container/before_render', 'symp_arfe_catch_output', 10, 1);
//add_action('elementor/frontend/container/before_render', 'arfe_handle_carousel', 10, 1);
add_action('elementor/frontend/column/before_render', 'symp_arfe_catch_output', 10, 1);
add_action('elementor/frontend/section/before_render', 'symp_arfe_catch_output', 10, 1);




function symp_arfe_catch_output( $section ) {
    // Catch output
    ob_start();
}

add_action('elementor/frontend/container/after_render', 'sympl_arfe_handle_wrapper', 10, 1);
add_action('elementor/frontend/column/after_render', 'sympl_arfe_handle_wrapper', 10, 1);
add_action('elementor/frontend/section/after_render', 'sympl_arfe_handle_wrapper', 10, 1);

// And then
function sympl_arfe_handle_wrapper( $section ) {
    // Collect output
    ...
    // make sure to echo it
    	if($repeater_name = arfe_check_if_repeater_class_in_widget($section)) {
			echo symp_arfe_prepare_content_by_repeater($content, $repeater_name);
	} else {
			echo $content;
		}
}

function symp_arfe_prepare_content_by_repeater($content, $repeater_name) {
		...
		foreach($repeater as $row) {
			...
			$row = symp_arfe_process_repeater_row($row);
...
add_action('arfe_repeater_row_content', 'symp_arfe_repeater_row_content_fn', 10, 2);
function symp_arfe_repeater_row_content_fn ($single_content, $row)
{
    foreach($row as $key => $value ) {
        $single_content = symp_arfe_replace_content($single_content, $key, $value);
    }
 ...
 */
function symp_arfe_is_translatepress_active() {
 ...
 */
function symp_arfe_translate_text($text) {
    if (symp_arfe_is_translatepress_active()) {
 ...
 */
function symp_arfe_process_repeater_row($row) {
    if (!is_array($row)) {
        return $row;
    }
    ...
    foreach ($row as $key => $value) {
        if (is_string($value)) {
            $processed_row[$key] = symp_arfe_translate_text($value);
        } else {
...
function symp_arfe_replace_content($content, $key, $value) {
    ...
    if (is_string($value)) {
        $value = symp_arfe_translate_text($value);
    }
...
add_action( 'elementor/frontend/widget/after_render', function( $widget ) {
    ...
    if($repeater_name && $widget->get_name() != "loop-carousel" && $widget->get_name() != "arfe_repeater_carousel") {
        ...
        if($widget->get_name() == "accordion" || $widget->get_name() == "toggle") {
            echo symp_arfe_prepare_accordion_by_repeater($widget_content_output, $repeater_name);
        } else if($widget->get_name() == "nested-accordion") {
            echo symp_arfe_prepare_nested_accordion_by_repeater($widget_content_output, $repeater_name);
        } else {
            echo symp_arfe_prepare_content_by_repeater($widget_content_output, $repeater_name);
        }
    } else {
 ...
 */
function symp_arfe_prepare_accordion_by_repeater($content, $repeater_name) {
    $repeater = get_field($repeater_name);
    if(!$repeater || count($repeater) == 0) {
        return "";
    }
 ...
 */
function symp_arfe_prepare_nested_accordion_by_repeater($content, $repeater_name) {
    $repeater = get_field($repeater_name);
    if(!$repeater || count($repeater) == 0) {
        return "";
    }
    ...
    foreach ($repeater as $row) {
        ...
        if ($new_title_text) {
            ...
            foreach ($row as $key => $value) {
                $title = str_replace("#" . $key, symp_arfe_translate_text($value), $title);
            }
        ...
        if ($new_content_element) {
            ...
            // Replace placeholders in content HTML
            foreach ($row as $key => $value) {
                $content_html = str_replace("#" . $key, symp_arfe_translate_text($value), $content_html);
            }
